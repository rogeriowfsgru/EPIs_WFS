// EPI Control WFS - SISTEMA COMPLETO CORRIGIDO
...
function renderizarUsuarios() {
    ...
    container.innerHTML = usuarios.map(u => `
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; padding:12px; border-bottom:1px solid var(--border-color)">
            <div><strong>${u.usuario}</strong> - ${u.nome}<br><small>${u.email} | Nível: ${u.nivel === "admin" ? "Administrador" : "Operador"}</small></div>
            ${u.id !== 1 && usuarioAtual?.nivel === "admin" && u.id !== usuarioAtual?.id ? `<div><button class="edit-btn" onclick="promoverAdmin(${u.id})">Promover</button><button class="delete-btn" onclick="deletarUsuario(${u.id})">Excluir</button></div>` : u.id === 1 ? '<span>Usuário padrão</span>' : ''}
        </div>
    `).join("");
}
...
function verificarSessao() {
    ...
    if (savedUser) {
        ...
        if (usuarioAtual.nivel === "admin") document.querySelectorAll(".admin-only").forEach(el => el.style.display = "flex");
        carregarDados();
        renderizarUsuarios();
        limparTudo();
    }
...
function inicializarEstoque() {
    listaEPIsBase.forEach(e => { if(!estoque[e.codigo]) estoque[e.codigo] = { codigo: e.codigo, descricao: e.descricao, quantidade: 10, emUso: 0, ultimaMov: null }; });
}

function carregarDados() {
    const se = localStorage.getItem("estoque_final12"); 
    if(se) estoque = JSON.parse(se);
    else { inicializarEstoque(); salvarDados(); }
...
function atualizarStats() {
    ...
    const el1 = document.getElementById("totalEPIs"); if(el1) el1.textContent = totalEPIs;
    const el2 = document.getElementById("totalMovs"); if(el2) el2.textContent = totalMovs;
    const el3 = document.getElementById("totalAtivos"); if(el3) el3.textContent = ativos.size;
}
...
function mostrarFeedback(msg, tipo) {
    ...
    setTimeout(()=>{ div.style.animation = 'slideOutRight 0.3s ease-out'; setTimeout(()=>{ div.style.display = "none"; div.style.animation = ''; }, 300); }, 3000);
}
...
document.querySelectorAll('.tab-btn').forEach(btn=>{
    btn.addEventListener('click',()=>{
        ...
        if(btn.dataset.tab==='requerimento') limparTudo();
    });
});
...
function limparTudo() {
    ...
    document.getElementById("fichaEPIs").style.display = "none";
    document.getElementById("autocompleteList").style.display = "none";
}

function selecionarColab(mat, nome, funcao, secao) {
    const campo = document.getElementById("matricula");
    if (campo) campo.value = mat;
    
    colaboradorSelecionado = { matricula: mat, nome: nome, funcao: funcao, secao: secao };
    ...
    mostrarFeedback(`✅ Colaborador ${nome} selecionado!`, "success");
}

function finalizarRequerimento() {
    let mat = colaboradorSelecionado ? colaboradorSelecionado.matricula : document.getElementById("matricula").value;
    
    if (!mat) {
        mostrarFeedback("⚠️ Selecione um colaborador!", "error");
        return;
    }
    
    if (carrinho.length === 0) {
        mostrarFeedback("⚠️ Adicione pelo menos um EPI ao carrinho!", "error");
        return;
    }
    
    let colab = colaboradores.find(c => c.matricula === mat);
    if (!colab && colaboradorSelecionado) colab = colaboradorSelecionado;
    
    if (!colab) {
        mostrarFeedback(`❌ Colaborador não encontrado!`, "error");
        return;
    }
    
    const dh = obterDataHora();
    const d = obterData();
    let ok = 0;
    ...
    for (const item of carrinho) {
        ...
        if (!est || est.quantidade < item.quantidade) {
            mostrarFeedback(`⚠️ Estoque insuficiente para ${item.descricao}`, "error");
            continue;
        }
        est.quantidade -= item.quantidade;
        est.emUso = (est.emUso || 0) + item.quantidade;
        est.ultimaMov = dh;
        movimentacoes.unshift({
            ...
            codigoEPI: item.codigo,
            descricaoEPI: item.descricao,
    ...
    if (ok > 0) {
        mostrarFeedback(`✅ REQUERIMENTO FINALIZADO! ${ok} item(s) retirado(s) por ${colab.nome}`, "success");
        carrinho = [];
        atualizarCarrinho();
        salvarDados();
        renderizarEstoque();
...
function cancelarRequerimento() {
    if (carrinho.length && confirm("Cancelar este requerimento?")) {
        carrinho = [];
        atualizarCarrinho();
        limparTudo();
        mostrarFeedback("Requerimento cancelado!", "success");
...
document.getElementById("matricula")?.addEventListener("input", function() {
    ...
    const filtrados = colaboradores.filter(c => c.matricula.toLowerCase().includes(val) || c.nome.toLowerCase().includes(val));
    
    if (filtrados.length > 0) {
        lista.innerHTML = filtrados.map(c => `<div class="autocomplete-item" onclick="selecionarColab('${c.matricula}','${c.nome.replace(/'/g, "\\'")}','${c.funcao || ''}','${c.secao || ''}')"><strong>${c.matricula}</strong> - ${c.nome}</div>`).join("");
...
function atualizarCarrinho() {
    ...
    if(!div) return;
    if(carrinho.length===0){ div.innerHTML='<div style="text-align:center;padding:20px;"><i class="fas fa-inbox"></i> Nenhum EPI adicionado</div>'; if(span) span.textContent="0 itens"; return; }
    const total = carrinho.reduce((s,i)=>s+i.quantidade,0);
    if(span) span.textContent = `${total} item(s)`;
    div.innerHTML = carrinho.map((item,idx)=>`<div class="carrinho-item"><div><strong>${item.descricao}</strong><br>Qtd: ${item.quantidade}</div><button class="btn-remover-carrinho" onclick="removerItem(${idx})">Remover</button></div>`).join("");
}
...
function renderizarEstoque() {
    ...
    for(const e of ordenados){
        ...
        let badgeClass = e.quantidade === 0 ? 'badge-danger' : (e.quantidade <= 5 ? 'badge-warning' : 'badge-success');
        row.insertCell(2).innerHTML = `<span class="badge ${badgeClass}">${e.quantidade} unidades</span>`;
        let statusText = e.quantidade === 0 ? 'Esgotado' : (e.quantidade <= 5 ? 'Baixo estoque' : 'Disponível');
        ...
        row.insertCell(6).innerHTML = `<button class="edit-btn" onclick="editarEPI('${e.codigo}')">Editar</button>`;
    }
}
...
function renderizarHistorico(filtro=""){
    ...
    if(dados.length===0){ cont.innerHTML='<div class="empty-state">Nenhum movimento encontrado</div>'; return; }
    cont.innerHTML = dados.map(m=>`<div class="historico-item"><strong>RETIRADA</strong> - ${m.dataHora}<br>EPI: ${m.descricaoEPI} - Qtd: ${m.quantidade}<br>Colaborador: ${m.nomeColab} (${m.matricula})</div>`).join("");
}

function renderizarColaboradores(busca=""){
    const tbody = document.getElementById("colaboradoresTableBody");
    if(!tbody) return;
    if(!busca){ tbody.innerHTML='<tr><td colspan="7">Digite para buscar</td></tr>'; return; }
    ...
    if(filtrados.length===0){ tbody.innerHTML='<tr><td colspan="7">Nenhum encontrado</td></tr>'; return; }
    tbody.innerHTML = "";
    filtrados.forEach(c => {
        ...
        row.insertCell(1).innerHTML = c.nome;
        row.insertCell(2).innerHTML = c.secao || '---';
        row.insertCell(3).innerHTML = c.dataAdmissao || '---';
        row.insertCell(4).innerHTML = c.funcao || '---';
        row.insertCell(5).innerHTML = `${epis.length} EPI(s)`;
        row.insertCell(6).innerHTML = `<button class="edit-btn" onclick="editarColab('${c.matricula}')">Editar</button><button class="delete-btn" onclick="deletarColab('${c.matricula}')">Excluir</button>`;
    });
}
...
document.getElementById("fileInput")?.addEventListener("change",e=>{
    ...
    reader.onload=ev=>{
        ...
        if(imp){ salvarDados(); document.getElementById("uploadInfo").innerHTML=`✅ ${imp} importados!`; mostrarFeedback(`✅ ${imp} colaboradores importados!`,"success"); renderizarColaboradores(""); document.getElementById("fileInput").value=""; }
